...
/// struct for passing parameters to the method [`stats_user_feedback_create`]
#[derive(Clone, Debug)]
pub struct StatsUserFeedbackCreateParams {
    pub user_feedback: crate::models::UserFeedback,
}

/// struct for typed errors of method [`stats_user_feedback_create`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum StatsUserFeedbackCreateError {
    UnknownValue(serde_json::Value),
}

pub async fn stats_user_feedback_create(
    configuration: &configuration::Configuration,
    params: StatsUserFeedbackCreateParams,
) -> Result<crate::models::UserFeedback, Error<StatsUserFeedbackCreateError>> {
    let local_var_configuration = configuration;

    // unbox the parameters
    let user_feedback = params.user_feedback;

    let local_var_client = &local_var_configuration.client;

    let local_var_uri_str = format!("{}/stats/user_feedback/", local_var_configuration.base_path);
    ...
    local_var_req_builder = local_var_req_builder.json(&user_feedback);
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        ...
        let local_var_entity: Option<StatsUserFeedbackCreateError> =
