...
/// struct for typed errors of method [`about_numbers_retrieve`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum AboutNumbersRetrieveError {
    UnknownValue(serde_json::Value),
}

pub async fn about_numbers_retrieve(
    configuration: &configuration::Configuration,
) -> Result<(), Error<AboutNumbersRetrieveError>> {
    ...
    let local_var_uri_str = format!("{}/api2/about-numbers/", local_var_configuration.base_path);
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        ...
        let local_var_entity: Option<AboutNumbersRetrieveError> =
